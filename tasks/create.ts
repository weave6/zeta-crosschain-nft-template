...
const main = async (args: any, hre: HardhatRuntimeEnvironment) => {
  ...
  const tx = await launchpad.createLaunchpad(
    args.nftAddress,
    signer.address,
    eth(args.price),
    args.amount,
  ...
  if (args.json) {
    ...
    console.log(`✅ "Create Launchpad" transaction has been broadcasted to ${hre.network.name}
...
task("create", "Create new launchpad", main)
